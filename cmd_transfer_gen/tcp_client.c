...
void parse_args(const char *args, const funcSpec *cmdSpec) {
    ...
    for (int n = 0; n < arg_n; n++) {
        ...
        uint8_t type_code = (uint8_t)arg_t;
        memcpy(send_buf + send_len, &type_code, sizeof(type_code));
        send_len += sizeof(type_code);
...
//afeSpiRawWrite(1, 0x0F2F, 0x25)   
        ...
        uint8_t n_args = cmdSpec -> num_args;
        uint8_t n_rets = cmdSpec -> num_rets;
        uint8_t arg_ret = (n_args<<3) | (n_rets);
        memcpy(send_buf + send_len, &arg_ret, sizeof(arg_ret));
        send_len += sizeof(arg_ret);
