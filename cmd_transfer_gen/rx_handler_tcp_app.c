...
void readWrapperFn(funcSpec, void** args, buffer_t*);
void genWrapperFn(funcSpec, void** args, buffer_t*);
                 			  ...
                 			  struct pbuf *p, err_t err)
{
    ...
    for (int i = 0; i < funcCount; i++){
   	 if(funcTable[i].ID == cmd_id){
   		 ...
   		 buffer_t toSend;
   		 funcTable[i].funcWrapper(funcTable[i], args, &toSend);
        	...
        	printf("buffer check!!\n");
        	for(uint32_t i = 0; i < toSend.len; i++)
            	printf("%02X ", toSend.buf[i]);
        	printf("\n");

   		 err_t werr = tcp_write(tpcb, toSend.buf, toSend.len, TCP_WRITE_FLAG_COPY);
...
void readWrapperFn(funcSpec fn, void** args, buffer_t* sendbuf){
	uint8_t* readVal;
	uint32_t ret;
	uint16_t dataArraySize = 1;
	if (fn.ID == FUNC_ID_AFE_SPI_BURST_READ) {
    	dataArraySize = *(uint16_t*)args[2];
	}
	readVal = malloc(dataArraySize*sizeof(uint8_t));
	args[(fn.num_args)] = &readVal;

	ret = fn.funcName(args);
	uint16_t arraySizeNet = htons(dataArraySize);
	uint8_t status = (uint8_t)ret;
	if (fn.ID == FUNC_ID_AFE_SPI_BURST_READ) {
    	//size, array, status
   	 sendbuf->len = sizeof(dataArraySize)+ dataArraySize*sizeof(uint8_t)+sizeof(uint8_t);
    	sendbuf->buf = malloc(sendbuf->len);
   	 memcpy(sendbuf->buf, &arraySizeNet, sizeof(dataArraySize));
   	 memcpy(sendbuf->buf + sizeof(dataArraySize), readVal, dataArraySize*sizeof(uint8_t));
   	 memcpy(sendbuf->buf + sizeof(dataArraySize) + dataArraySize*sizeof(uint8_t), &status, sizeof(uint8_t));
	}
	else {
   	 //data, status
   	 sendbuf->len = 2*sizeof(uint8_t);
    	memcpy(sendbuf->buf, readVal, dataArraySize);
    	memcpy(sendbuf->buf + dataArraySize, &status, sizeof(uint8_t));
	}
	free(readVal);
}

void genWrapperFn(funcSpec fn, void** args, buffer_t* sendbuf){
	uint32_t ret;
	ret = fn.funcName(args);
	uint8_t status = (uint8_t)ret;
	sendbuf->len = sizeof(uint8_t);
	sendbuf->buf = malloc(sendbuf->len);
	memcpy(sendbuf->buf, &status, sizeof(uint8_t));
}
