...
uint32_t afeSpiRawWrite(uint8_t afeInst, uint16_t addr, uint8_t data)
{
    ...
    xil_printf("%d %d", rxBuf[0], rxBuf[1]);
...
uint32_t afeSpiRawWriteMulti(uint8_t afeInstSel, uint16_t addr, uint8_t data)
{
    ...
    for (uint8_t i = 0; i < 8; i++) {
        if (afeInstSel & (1 << i)) {
            XSpiPs_SetSlaveSelect(&SpiInstance, i);
            int Status = XSpiPs_PolledTransfer(&SpiInstance, txBuf, rxBuf, 2);
            if (Status != XST_SUCCESS)
                return TI_AFE_RET_EXEC_FAIL;
        }
    }
    XSpiPs_SetSlaveSelect(&SpiInstance, 0xF); // Deselect all slaves
    return TI_AFE_RET_EXEC_PASS;
}
...
uint32_t afeSpiRawReadMulti(uint8_t afeInstSel, uint16_t addr, uint8_t *readVal)
{
    ...
    for (uint8_t i = 0; i < 8; i++) {
        if (afeInstSel & (1 << i)) {
            ...
            readVal[i] = 0; // As per requirements, zero for unselected slaves
        }
    }
    XSpiPs_SetSlaveSelect(&SpiInstance, 0xF); // Deselect all slaves
    return TI_AFE_RET_EXEC_PASS;
...
{
    uint8_t txBuf[dataArraySize * 2], rxBuf[dataArraySize * 2];
    for (uint16_t i = 0, j = 0; i < dataArraySize; i++, j += 2) {
        txBuf[j] = (addr + i) & 0x7F;
        txBuf[j + 1] = data[i];
    }
    ...
    for (uint8_t i = 0; i < 8; i++) {
        if (afeInstSel & (1 << i)) {
            XSpiPs_SetSlaveSelect(&SpiInstance, i);
            int Status = XSpiPs_PolledTransfer(&SpiInstance, txBuf, rxBuf, dataArraySize * 2);
            if (Status != XST_SUCCESS)
                return TI_AFE_RET_EXEC_FAIL;
        }
    }
